	...
	public class UIParticleSystem : MaskableGraphic
	{
		...
		protected bool Initialize()
		{
			...
			if (_pSystem == null)
			{
				...
				if (material == null)
				{
					Shader defaultShader = Shader.Find("UI/Additive");
				...
				if ((bool)_currentMaterial && _currentMaterial.HasProperty(MainTex))
				{
			...
			_textureSheetAnimationFrameSize = new Vector2(1f / (float)_textureSheetAnimation.numTilesX, 1f / (float)_textureSheetAnimation.numTilesY);
			return true;
		}
		...
		protected override void OnPopulateMesh(VertexHelper vh)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				float rotation = (0f - particle.rotation) * (3.14159265f / 180f);
				float rotation2 = rotation + 3.14159265f / 2f;
				...
				if (_textureSheetAnimation.enabled)
				{
					float frameProgress = 1f - particle.remainingLifetime / particle.startLifetime;
					if (_textureSheetAnimation.frameOverTime.curveMin != null)
					{
						frameProgress = _textureSheetAnimation.frameOverTime.curveMin.Evaluate(1f - particle.remainingLifetime / particle.startLifetime);
					}
					else if (_textureSheetAnimation.frameOverTime.curve != null)
					{
						frameProgress = _textureSheetAnimation.frameOverTime.curve.Evaluate(1f - particle.remainingLifetime / particle.startLifetime);
					}
					else if (_textureSheetAnimation.frameOverTime.constant > 0f)
					{
						frameProgress = _textureSheetAnimation.frameOverTime.constant - particle.remainingLifetime / particle.startLifetime;
					}
					frameProgress = Mathf.Repeat(frameProgress * (float)_textureSheetAnimation.cycleCount, 1f);
					int frame = 0;
					switch (_textureSheetAnimation.animation)
					{
					case ParticleSystemAnimationType.WholeSheet:
						frame = Mathf.FloorToInt(frameProgress * (float)_textureSheetAnimationFrames);
						break;
					case ParticleSystemAnimationType.SingleRow:
					{
						frame = Mathf.FloorToInt(frameProgress * (float)_textureSheetAnimation.numTilesX);
					...
					particleUV.x = (float)(frame % _textureSheetAnimation.numTilesX) * _textureSheetAnimationFrameSize.x;
					particleUV.y = 1f - (float)Mathf.FloorToInt((float)frame / (float)_textureSheetAnimation.numTilesX) * _textureSheetAnimationFrameSize.y;
				...
				else
				{
					...
					Vector2 up = new Vector2(Mathf.Cos(rotation2), Mathf.Sin(rotation2)) * size;
